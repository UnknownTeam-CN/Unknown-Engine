...
// ResultsScreen 鏁版嵁浼犻€掔粨鏋?鈥?Unknown Engine
typedef ResultsData = {
	var sicks:Int;
	var goods:Int;
	var bads:Int;
	...
	var accuracy:Float;
	var ratingFC:String;
	var songLength:Float; // Unknown Engine: 传递给 HitGraph 使用
}

// Replay 帧数据 — 每帧的按键状态
typedef ReplayFrameData = {
	var t:Float;       // Conductor.songPosition (ms)
	var k:Array<Bool>; // [left, down, up, right] 是否按住
}
...
class PlayState extends MusicBeatState
{
	...
	private static function get_songLength():Float {
		return instance != null ? instance._songLength : 0;
	}
	private var _songLength:Float = 0; 
	...
	public static var rep:backend.Replay = null;
	public var replayAna:Analysis = new Analysis();
	public var replayChar:String = "bf";
	public var replayInputFrames:Array<ReplayFrameData> = []; 
	public var replayInputIndex:Int = 0;                      
	public static var beatHitData:Array<Array<Dynamic>> = [];

	// Silhouette event state — Lua-style: white overlay + char/HUD color tween
	public static var silhouetteActive:Bool = false;
	public var silhouetteWhiteBg:FlxSprite = null;

	// Lyric event state — native integration of Avis's Lyric2 & Lyric scripts
	// lyric2Text: for 'Lyric2' event, lyricText: for 'Lyric - Set Text' event
	public var lyric2Text:FlxText = null;
	...
	public static var campaignHighestCombo:Int = 0;
	...
	override public function create()
	{
		...
		if (loadRep && rep != null && rep.replay != null)
		{
			try
			{
				var replayPath:String = rep._replayPath;
				if (replayPath == null || replayPath == "" || !sys.FileSystem.exists(replayPath))
				{
					var songLowercase:String = Paths.formatToSongPath(rep.replay.songName);
					var searchDir:String = 'replays/jsons/$songLowercase';
					if (sys.FileSystem.exists(searchDir))
					{
						for (file in sys.FileSystem.readDirectory(searchDir))
						{
							if (file.indexOf('ue_') == 0 && StringTools.endsWith(file, ".json"))
							{
								replayPath = '$searchDir/$file';
								rep._replayPath = replayPath;
								break;
							}
						}
					}
				...
				if (replayPath != null && replayPath != "" && sys.FileSystem.exists(replayPath))
				{
					var jsonStr:String = sys.io.File.getContent(replayPath);
					var jsonData:Dynamic = haxe.Json.parse(jsonStr);
					if (jsonData.inputs != null)
					{
						var rawInputs:Array<Dynamic> = jsonData.inputs;
						for (raw in rawInputs)
						{
							var keys:Array<Bool> = [];
							for (vk in (raw.k : Array<Dynamic>))
								keys.push(vk);
							replayInputFrames.push({t: raw.t, k: keys});
						}
			...
			{
				trace('Failed to load replay inputs: $e');
			}
		}
	...
	function startSong():Void
	{
		...
		_songLength = FlxG.sound.music.length;
	...
	private function generateSong():Void
	{
		...
		// Replay 鍥炴斁鍒濆鍖?鈥?Unknown Engine
		if (loadRep && rep != null)
		{
			ClientPrefs.data.downScroll = rep.replay.isDownscroll;
			SONG.speed = rep.replay.noteSpeed;
		}
		try
		{
	...
	public function triggerEvent(eventName:String, value1:String, value2:String, strumTime:Float) {
		...
		switch(eventName) {
			...
			case 'Silhouette':
				// Lua-style implementation: white overlay alpha tween + char/HUD color tween
				// Value 1: 'a' = enable (BG white, chars/HUD black), 'b' = disable (restore)
				//          Also supports '1'/'0' and 'true'/'false'
				// Value 2: fade duration in seconds (default 1.0)
				var v1:String = (value1 != null) ? value1.trim().toLowerCase() : '';
				var duration:Float = (flValue2 != null && flValue2 > 0) ? flValue2 : 1.0;
				duration /= playbackRate;

				var enable:Bool = (v1 == 'a' || v1 == '1' || v1 == 'true');
				var disable:Bool = (v1 == 'b' || v1 == '0' || v1 == 'false');
				if (!enable && !disable) return;

				// Create white overlay sprite if not yet created
				if (silhouetteWhiteBg == null)
				{
					silhouetteWhiteBg = new FlxSprite().makeGraphic(Std.int(FlxG.width * 2), Std.int(FlxG.height * 2), FlxColor.WHITE);
					silhouetteWhiteBg.scrollFactor.set(0, 0);
					silhouetteWhiteBg.alpha = 0;
					silhouetteWhiteBg.screenCenter();
					// Insert before first character group so chars are drawn on top
					var insertIdx:Int = -1;
					for (grp in [gfGroup, dadGroup, boyfriendGroup])
					{
						if (grp != null)
						{
							var idx:Int = members.indexOf(grp);
							if (idx >= 0) { insertIdx = idx; break; }
						}
					}
					if (insertIdx >= 0)
						insert(insertIdx, silhouetteWhiteBg);
					else
						add(silhouetteWhiteBg); // fallback: add at end
				}

				// Cancel any existing silhouette tweens on affected objects
				FlxTween.cancelTweensOf(silhouetteWhiteBg, ["alpha"]);
				FlxTween.cancelTweensOf(boyfriend, ["color"]);
				FlxTween.cancelTweensOf(dad, ["color"]);
				if (gf != null) FlxTween.cancelTweensOf(gf, ["color"]);
				FlxTween.cancelTweensOf(iconP1, ["color"]);
				FlxTween.cancelTweensOf(iconP2, ["color"]);
				if (healthBar != null)
				{
					FlxTween.cancelTweensOf(healthBar.bg, ["color"]);
					FlxTween.cancelTweensOf(healthBar.leftBar, ["color"]);
					FlxTween.cancelTweensOf(healthBar.rightBar, ["color"]);
				}

				if (enable)
				{
					silhouetteActive = true;
					// White overlay: fade in to cover BG
					FlxTween.tween(silhouetteWhiteBg, {alpha: 1}, duration, {ease: FlxEase.linear});
					// Characters: fade to black silhouette
					FlxTween.color(boyfriend, duration, boyfriend.color, FlxColor.BLACK, {ease: FlxEase.linear});
					FlxTween.color(dad, duration, dad.color, FlxColor.BLACK, {ease: FlxEase.linear});
					if (gf != null) FlxTween.color(gf, duration, gf.color, FlxColor.BLACK, {ease: FlxEase.linear});
					// HUD icons: fade to black
					FlxTween.color(iconP1, duration, iconP1.color, FlxColor.BLACK, {ease: FlxEase.linear});
					FlxTween.color(iconP2, duration, iconP2.color, FlxColor.BLACK, {ease: FlxEase.linear});
					// Health bar: fade all parts to black
					if (healthBar != null)
					{
						FlxTween.color(healthBar.bg, duration, healthBar.bg.color, FlxColor.BLACK, {ease: FlxEase.linear});
						FlxTween.color(healthBar.leftBar, duration, healthBar.leftBar.color, FlxColor.BLACK, {ease: FlxEase.linear});
						FlxTween.color(healthBar.rightBar, duration, healthBar.rightBar.color, FlxColor.BLACK, {ease: FlxEase.linear});
					}
				}
				else // disable
				{
					...
					FlxTween.tween(silhouetteWhiteBg, {alpha: 0}, duration, {ease: FlxEase.linear});
					// Characters: restore (WHITE = no color tint)
					FlxTween.color(boyfriend, duration, boyfriend.color, FlxColor.WHITE, {ease: FlxEase.linear});
					FlxTween.color(dad, duration, dad.color, FlxColor.WHITE, {ease: FlxEase.linear});
					if (gf != null) FlxTween.color(gf, duration, gf.color, FlxColor.WHITE, {ease: FlxEase.linear});
					// HUD icons: restore
					FlxTween.color(iconP1, duration, iconP1.color, FlxColor.WHITE, {ease: FlxEase.linear});
					FlxTween.color(iconP2, duration, iconP2.color, FlxColor.WHITE, {ease: FlxEase.linear});
					// Health bar bg: restore (shows original image)
					if (healthBar != null)
					{
						FlxTween.color(healthBar.bg, duration, healthBar.bg.color, FlxColor.WHITE, {ease: FlxEase.linear});
						// leftBar/rightBar: tween to WHITE, then reload proper colors via onComplete
						FlxTween.color(healthBar.leftBar, duration, healthBar.leftBar.color, FlxColor.WHITE,
							{ease: FlxEase.linear, onComplete: function(t:FlxTween) { reloadHealthBarColors(); }});
						FlxTween.color(healthBar.rightBar, duration, healthBar.rightBar.color, FlxColor.WHITE,
							{ease: FlxEase.linear});
					}
				}

			case 'Lyric2':
				// Display lyrics with fade in/out (from Avis's Lyric2.lua)
				// Value 1: text to show (empty = hide)
				var fadeTime:Float = Conductor.crochet / 1500;
				var yOff:Float = 50;

				if (lyric2Text == null)
				{
					lyric2Y = ClientPrefs.data.downScroll ? 570 : 520;
					lyric2Text = new FlxText(-50, lyric2Y, 1000, "", 34);
					lyric2Text.setFormat(Paths.font("vcr.ttf"), 34, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
					lyric2Text.borderSize = 2;
				...
				{
					// Show text: start from below + fade in
					lyric2Text.y = lyric2Y + yOff;
					lyric2Text.alpha = 0;
					lyric2Text.text = value1;
					FlxTween.tween(lyric2Text, {y: lyric2Y}, fadeTime * 2, {ease: FlxEase.quadOut});
					FlxTween.tween(lyric2Text, {alpha: 1}, fadeTime * 2, {ease: FlxEase.quadOut});
				}
				else
				{
					// Hide text: move down + fade out
					FlxTween.tween(lyric2Text, {y: lyric2Text.y + yOff}, fadeTime, {ease: FlxEase.quadIn});
					FlxTween.tween(lyric2Text, {alpha: 0}, fadeTime, {ease: FlxEase.quadIn});
				}

			case 'Lyric - Set Text':
				// Display lyrics with fade in/out (from Avis's Lyric - Set Text.lua)
				// Value 1: text to show (empty = hide)
				var fadeTime:Float = Conductor.crochet / 1500;
				var yOff:Float = 50;
				...
				if (lyricText == null)
				{
					lyricTextY = ClientPrefs.data.downScroll ? 520 : 560;
					lyricText = new FlxText(-50, lyricTextY, 1000, "", 34);
					lyricText.setFormat(Paths.font("vcr.ttf"), 34, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
				...
				{
					// Show text: start from below + fade in
					lyricText.y = lyricTextY + yOff;
					lyricText.alpha = 0;
					lyricText.text = value1;
					FlxTween.tween(lyricText, {y: lyricTextY}, fadeTime * 2, {ease: FlxEase.quadOut});
					FlxTween.tween(lyricText, {alpha: 1}, fadeTime * 2, {ease: FlxEase.quadOut});
				}
				else
				{
					// Hide text: move down + fade out
					FlxTween.tween(lyricText, {y: lyricText.y + yOff}, fadeTime, {ease: FlxEase.quadIn});
		...
		case 'Lyric - Set Color':
			...
			case 'Shader':
				#if (!flash && sys)
				var shaderName:String = (value1 != null) ? value1.trim() : '';
				var enable:Bool = (value2 == '1' || (value2 != null && value2.toLowerCase() == 'true'));

				// If value1 is empty, remove ALL active event shaders regardless of value2
				if (shaderName.length <= 0)
				{
					for (key in activeShaderEvents.keys())
					{
						var entry = activeShaderEvents.get(key);
						if (entry != null)
						{
							removeShaderFromCamera(entry.filter);
							activeShaderEvents.remove(key);
						}
					}
					return;
				}
				...
				if (enable)
				{
					// Enable shader: init it if needed, then apply to camGame
					if (!runtimeShaders.exists(shaderName) && !initLuaShader(shaderName))
					{
						FlxG.log.warn('Shader event: Shader "$shaderName" is missing!');
						return;
					}

					// If already active, skip
					if (activeShaderEvents.exists(shaderName))
						return;

					var arr:Array<String> = runtimeShaders.get(shaderName);
					var shaderObj:shaders.ErrorHandledShader.ErrorHandledRuntimeShader = new shaders.ErrorHandledShader.ErrorHandledRuntimeShader(shaderName, arr[0], arr[1]);
					var filterObj:openfl.filters.ShaderFilter = new openfl.filters.ShaderFilter(shaderObj);
					activeShaderEvents.set(shaderName, {shader: shaderObj, filter: filterObj});

					addShaderToCamera(filterObj);
				}
				else
				{
					// Disable specific shader
					if (activeShaderEvents.exists(shaderName))
					{
						var entry = activeShaderEvents.get(shaderName);
						if (entry != null)
							removeShaderFromCamera(entry.filter);
						activeShaderEvents.remove(shaderName);
					}
				}
				#else
	...
	public function endSong()
	{
		...
		else if (isStoryMode)
		{
				...
				if (highestCombo > campaignHighestCombo)
					campaignHighestCombo = highestCombo;
			...
			replayAna = new Analysis();
				...
				{
					// 还有下一首歌 — 继续连打
					var nextSong:String = storyPlaylist[0];
					Song.loadFromJson(nextSong.toLowerCase() + difficultySuffix, nextSong.toLowerCase());
					FlxG.sound.music.stop();
				...
				{
					// 最后一首歌结束 — 用累积数据填充 resultsData
					var totalPlayed:Int = campaignSicks + campaignGoods + campaignBads + campaignShits + campaignMisses;
					var totalNotesHit:Float = campaignSicks * 1.0 + campaignGoods * 0.75 + campaignBads * 0.5 + campaignShits * 0.25;
					var totalAccuracy:Float = (totalPlayed > 0) ? Math.min(1, totalNotesHit / totalPlayed) : 0;

					// 重新计算总 ratingFC
					var totalRatingFC:String;
					if (campaignMisses == 0 && campaignBads == 0 && campaignShits == 0 && campaignGoods == 0)
						totalRatingFC = "MFC";
					else if (campaignMisses == 0 && campaignBads == 0 && campaignShits == 0)
						totalRatingFC = "GFC";
					else if (campaignMisses == 0)
						totalRatingFC = "FC";
					...
					resultsData = {
						...
						highestCombo: campaignHighestCombo,
						score: campaignScore,
						accuracy: totalAccuracy,
						ratingFC: totalRatingFC,
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		// KE/PE MS显示支持
		var signedDiff:Float;
		var noteDiff:Float;
		var daRating:Rating;

		// Replay: 使用原始记录的判定而非当前时机重新计算
		if (loadRep && rep != null && rep.replay != null && rep.replay.songNotes != null && rep.replay.songJudgements != null) {
			var idx:Int = findByTimeIndex(note.strumTime);
			if (idx >= 0 && idx < rep.replay.songJudgements.length && idx < rep.replay.songNotes.length) {
				var entry:Array<Dynamic> = rep.replay.songNotes[idx];
				signedDiff = entry[3]; // 原始记录的 noteDiff
				noteDiff = Math.abs(signedDiff);
				var judgeName:String = rep.replay.songJudgements[idx];
				daRating = null;
				...
				if (daRating == null) daRating = Conductor.judgeNote(ratingsData, noteDiff / playbackRate);
			} else {
				signedDiff = note.strumTime - Conductor.songPosition + ClientPrefs.data.ratingOffset;
				noteDiff = Math.abs(signedDiff);
		...
		// MS鍋忓樊鏄剧ず 鈥?Unknown Engine
		var showMs:Bool = true; // 鍏抽棴鍙敼涓?false
		if (showMs && note != null)
		{
			var msSigned:Float = Math.round(signedDiff * 10) / 10;
			var msStr:String = (msSigned >= 0 ? '+' : '') + Std.string(msSigned) + 'ms';
			var msColor:Int = 0xFFFFFFFF;
			switch(daRating.name)
			{
				case 'sick':  msColor = 0xFF00FFFF;
				case 'good':  msColor = 0xFF00FF00;
				case 'bad':   msColor = 0xFFFF0000;
				case 'shit':  msColor = 0xFF8B0000;
				default:      msColor = 0xFFFFFFFF;
			}
			var msText:FlxText = new FlxText(0, 0, 0, msStr, 16);
			msText.setFormat(Paths.font('vcr.ttf'), 16, msColor, LEFT);
			msText.screenCenter();
			msText.x = placement - 40 + 50;
			msText.y -= 60;
			...
			msText.visible = (!ClientPrefs.data.hideHud && showRating);
			comboGroup.add(msText);
			FlxTween.tween(msText, {alpha: 0}, 0.2 / playbackRate, {
				onComplete: function(tween:FlxTween) { msText.destroy(); },
				startDelay: Conductor.crochet * 0.002 / playbackRate
			});
		}
	...
	{
		if(cpuControlled || !startedCountdown || paused || key < 0 || key >= playerStrums.length) return; // 移除 loadRep 检查以允许回放释放按键
	...
	private function keysCheck():Void
	{
		...
		// TO DO: Find a better way to handle controller inputs, this should work for now
		// Replay: release 已在上面模拟，此处跳过
		if (!loadRep && (controls.controllerMode || strumsBlocked.contains(true)) && releaseArray.contains(true))
	...
	function noteMissCommon(direction:Int, note:Note = null)
	{
		...
		songScore -= 10;
		// Replay 褰曞埗 鈥?Unknown Engine
		if (!cpuControlled && !loadRep)
		{
			var _strumTime:Float = Conductor.songPosition;
			saveNotes.push([_strumTime, 0, direction, -1.0]);
			saveJudges.push("miss");
			beatHitData.push([-1.0, "miss", _strumTime]);
		}
	...
	public function goodNoteHit(note:Note):Void
	{
		...
		if(!note.hitCausesMiss) //Common notes
		{
			...
			if (!cpuControlled && !loadRep)
			{
				var _noteDiff:Float = note.strumTime - Conductor.songPosition + ClientPrefs.data.ratingOffset;
				saveNotes.push([note.strumTime, note.sustainLength, note.noteData, _noteDiff]);
				saveJudges.push(note.rating);
				beatHitData.push([_noteDiff, note.rating, note.strumTime]);
			}
			}
	...
	// Replay 辅助函数: 在 replay 数据中按 strumTime 查找音符
	function findByTime(time:Float):Array<Dynamic>
	{
		if (rep == null || rep.replay == null || rep.replay.songNotes == null) return null;
	...
	// Replay 辅助函数: 在 replay 数据中按 strumTime 查找索引
	function findByTimeIndex(time:Float):Int
	{
		if (rep == null || rep.replay == null || rep.replay.songNotes == null) return -1;
	...
	// 比较两个 Bool 数组是否相等（用于去重录制）
	function arrayEqualsBool(a:Array<Bool>, b:Array<Bool>):Bool
	{
	...
	override function destroy() {
		...
		FlxG.camera.setFilters([]);
		activeShaderEvents.clear(); // cleanup shader event state
	...
	#if (!flash && sys)
	/**
	 * Adds a ShaderFilter to camGame's filter list.
	 * Preserves any existing filters (e.g. from stage scripts).
	 */
	function addShaderToCamera(filterObj:openfl.filters.ShaderFilter)
	{
		if (camGame == null) return;
		var currentFilters:Array<openfl.filters.BitmapFilter> = [];
		if (camGame.filters != null)
		{
			for (f in camGame.filters)
				currentFilters.push(f);
		}
		currentFilters.push(filterObj);
		camGame.setFilters(currentFilters);
	}

	/**
	 * Removes a specific ShaderFilter from camGame's filter list.
	 */
	function removeShaderFromCamera(filterObj:openfl.filters.ShaderFilter)
	{
		if (camGame == null) return;
		var currentFilters:Array<openfl.filters.BitmapFilter> = [];
		if (camGame.filters != null)
		...
		camGame.setFilters(currentFilters.length > 0 ? currentFilters : null);
	}
	#end
