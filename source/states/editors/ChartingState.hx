...
class ChartingState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	function analyzeBPMFromAudio()
	{
		...
		@:privateAccess
		{
			...
			{
				showOutput('No audio loaded to analyze.', true);
				return;
			}
		...
		{
			showOutput('Audio buffer is empty.', true);
			return;
		}
		...
		{
			showOutput('Audio too short to analyze.', true);
			return;
		}
		...
		if(maxLag <= minLag)
		{
			showOutput('Cannot determine BPM from audio.', true);
			return;
		}
		...
		{
			showOutput('BPM analysis failed: out of range.', true);
			return;
		}
		...
		showOutput('BPM detected from audio: $detectedBPM');
		#else
		showOutput('Audio analysis not available on this target.', true);
		#end
	}
	...
	function addSongTab()
	{
		...
		tapButton = new PsychUIButton(objX, objY + 25, 'TAP', function()
		{
			...
			if(tapTimestamps.length >= 4)
			{
				...
				PlayState.SONG.bpm = detectedBPM;
				adaptNotesToNewTimes(oldTimes);
				showOutput('BPM detected from TAP: $detectedBPM (${tapTimestamps.length} taps)');
			}
			else
			{
				showOutput('TAP ${tapTimestamps.length}/4...');
			}
		}, 60);

		analyzeButton = new PsychUIButton(objX + 70, objY + 25, 'ANALYSE', function()
		{
			analyzeBPMFromAudio();
		}, 80);
