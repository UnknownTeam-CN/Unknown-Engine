...

{
	...
	[
		...
		['Silhouette', "Shadow silhouette effect (like Indie Cross / Mind Games).\nValue 1: 'a' = enable (BG white, chars/HUD black), 'b' = disable (restore)\nValue 2: Fade duration in seconds (Default: 1.0)"],

		['Shader', "Apply a runtime shader to camGame.\nValue 1: Shader name (frag filename without .frag)\nValue 2: 1/0 = Enable/Disable\nIf Value 1 is empty, removes ALL active event shaders"],

		['Lyric2', "Display lyrics on screen with fade in/out animation.\nValue 1: Text to show (leave empty to hide/dismiss current lyrics)\nCreated by Avis."],

		['Lyric2 Color', "Change text color for Lyric2.\nValue 1: Hex color (e.g. ff0000 = red). Leave empty for white.\nCreated by Avis."],
...

			{
...

				{
					...
					if(selectedNotes.length > 0)
						...
						showOutput(Language.getPhrase('chart_msg_move_notes', 'You must select notes to move them!'), true);
...

	{
...

		{
			...
			selectedEventText.text = Language.getPhrase('chart_selected_event', 'Selected Event: {1} / {2}').replace('{1}', Std.string(curEventSelected + 1)).replace('{2}', Std.string(eventNote.events.length));
...

	{
...

		{
...

			{
				...
				showOutput(Language.getPhrase('chart_msg_no_audio', 'No audio loaded to analyze.'), true);
...

		{
			...
			showOutput(Language.getPhrase('chart_msg_audio_empty', 'Audio buffer is empty.'), true);
		...
		// Assume 16-bit samples (standard for FNF audio), consistent with existing waveform code

		var frameSize:Int = 2 * channels; // 2 bytes per channel (16-bit)



		// Total sample frames

		var totalFrames:Int = Std.int(bytes.length / frameSize);

		if(totalFrames < sampleRate) // less than 1 second of audio

		{

			showOutput(Language.getPhrase('chart_msg_audio_short', 'Audio too short to analyze.'), true);
		...
		// Analyze the first ~30 seconds max for speed

		var maxFrames:Int = Std.int(Math.min(totalFrames, sampleRate * 30));



		// Use energy-based onset detection with autocorrelation

		// Step 1: Compute energy in 1024-sample windows

		var windowSize:Int = 1024;

		var numWindows:Int = Std.int(maxFrames / windowSize);
...

		{
			...
			var start:Int = w * windowSize;

			for(i in 0...windowSize)

			{

				var frameIdx:Int = start + i;

				if(frameIdx >= totalFrames) break;



				// Read 16-bit signed sample (first channel), consistent with waveform code

				var byteOffset:Int = frameIdx * frameSize;

				if(byteOffset + 2 > bytes.length) break;



				var byte:Int = bytes.getUInt16(byteOffset);

				if(byte > 65535 / 2) byte -= 65535; // Convert to signed

				var normalized:Float = byte / 32768.0;
		...
		// Step 2: Autocorrelation to find the dominant period

		// Window time in seconds

		var windowTime:Float = windowSize / sampleRate;

		// BPM search range: 60-300 BPM

		var minLag:Int = Std.int(Math.max(1, Std.int(60.0 / windowTime / 300.0))); // 300 BPM

		var maxLag:Int = Std.int(Math.min(Std.int(numWindows / 2), Std.int(60.0 / windowTime / 60.0))); // 60 BPM
...

		{
			...
			showOutput(Language.getPhrase('chart_msg_bpm_fail', 'Cannot determine BPM from audio.'), true);
		...
		// Convert lag (in energy windows) to BPM

		var periodSeconds:Float = bestLag * windowTime;

		var detectedBPM:Float = 60.0 / periodSeconds;



		// Clamp to typical range

		while(detectedBPM > 300) detectedBPM /= 2;
		...
		if(detectedBPM < 1 || detectedBPM > 400)

		{

			showOutput(Language.getPhrase('chart_msg_bpm_range', 'BPM analysis failed: out of range.'), true);
		...
		bpmStepper.value = detectedBPM;

		var oldTimes:Array<Float> = cachedSectionTimes.copy();

		PlayState.SONG.bpm = detectedBPM;

		adaptNotesToNewTimes(oldTimes);

		showOutput(Language.getPhrase('chart_msg_bpm_detected', 'BPM detected from audio: {1}').replace('{1}', Std.string(detectedBPM)));

		#else

		showOutput(Language.getPhrase('chart_msg_bpm_unavailable', 'Audio analysis not available on this target.'), true);
...

	{
		...
		DiscordClient.changePresence(Language.getPhrase('discord_chart_editor', 'Chart Editor'), Language.getPhrase('discord_chart_detail', 'Song: {1}').replace('{1}', PlayState.SONG.song));
...

	{
		...
		var txt = new FlxText(objX, objY, 280, Language.getPhrase('chart_audio_warning', "Any options here won't actually affect gameplay!"));
		...
		mouseSnapCheckBox = new PsychUICheckBox(objX, objY, Language.getPhrase('chart_mouse_scroll_snap', 'Mouse Scroll Snap'), 100, function() chartEditorSave.data.mouseScrollSnap = mouseSnapCheckBox.checked);

		mouseSnapCheckBox.checked = chartEditorSave.data.mouseScrollSnap;



		ignoreProgressCheckBox = new PsychUICheckBox(objX + 150, objY, Language.getPhrase('chart_ignore_progress', 'Ignore Progress Warnings'), 100, function() chartEditorSave.data.ignoreProgressWarns = ignoreProgressCheckBox.checked);
		...
		instMuteCheckBox = new PsychUICheckBox(objX, objY, Language.getPhrase('chart_mute', 'Mute'), 60, updateAudioVolume);

		playerMuteCheckBox = new PsychUICheckBox(objX + 100, objY, Language.getPhrase('chart_mute', 'Mute'), 60, updateAudioVolume);
		...
		tab_group.add(new FlxText(hitsoundPlayerStepper.x, hitsoundPlayerStepper.y - 15, 100, Language.getPhrase('chart_hitsound_player', 'Hitsound (Player):')));

		tab_group.add(new FlxText(hitsoundOpponentStepper.x, hitsoundOpponentStepper.y - 15, 100, Language.getPhrase('chart_hitsound_opp', 'Hitsound (Opp.):')));

		tab_group.add(new FlxText(metronomeStepper.x, metronomeStepper.y - 15, 100, Language.getPhrase('chart_metronome', 'Metronome:')));
		...
		tab_group.add(new FlxText(instVolumeStepper.x, instVolumeStepper.y - 15, 100, Language.getPhrase('chart_inst_volume', 'Inst. Volume:')));

		tab_group.add(new FlxText(playerVolumeStepper.x, playerVolumeStepper.y - 15, 100, Language.getPhrase('chart_main_vocals', 'Main Vocals:')));

		tab_group.add(new FlxText(opponentVolumeStepper.x, opponentVolumeStepper.y - 15, 100, Language.getPhrase('chart_opp_vocals', 'Opp. Vocals:')));
...

	{
		...
		noRGBCheckBox = new PsychUICheckBox(objX, objY, Language.getPhrase('chart_disable_rgb', 'Disable Note RGB'), 100, updateNotesRGB);
...

		{
...

			{
...

				{
					...
					if(noteTextureInputText.text.trim().length > 0) showOutput(Language.getPhrase('chart_msg_texture_loaded', 'Reloaded notes to: "{1}"').replace('{1}', textureLoad));

					else showOutput(Language.getPhrase('chart_msg_texture_default', 'Reloaded notes to default texture'));

					

				}

				else showOutput(Language.getPhrase('chart_msg_texture_error', 'ERROR: "{1}" not found.').replace('{1}', textureLoad), true);
		...
		tab_group.add(new FlxText(gameOverCharDropDown.x, gameOverCharDropDown.y - 15, 120, Language.getPhrase('chart_gameover_char', 'Game Over Character:')));

		tab_group.add(new FlxText(gameOverSndInputText.x, gameOverSndInputText.y - 15, 180, Language.getPhrase('chart_gameover_death_sound', 'Game Over Death Sound (sounds/):')));

		tab_group.add(new FlxText(gameOverLoopInputText.x, gameOverLoopInputText.y - 15, 180, Language.getPhrase('chart_gameover_loop_music', 'Game Over Loop Music (music/):')));

		tab_group.add(new FlxText(gameOverRetryInputText.x, gameOverRetryInputText.y - 15, 180, Language.getPhrase('chart_gameover_retry_music', 'Game Over Retry Music (music/):')));
		...
		tab_group.add(new FlxText(noteTextureInputText.x, noteTextureInputText.y - 15, 100, Language.getPhrase('chart_note_texture', 'Note Texture:')));

		tab_group.add(new FlxText(noteSplashesInputText.x, noteSplashesInputText.y - 15, 120, Language.getPhrase('chart_note_splashes_texture', 'Note Splashes Texture:')));
...

	{
...

		{
...

			{
				...
				else showOutput(Language.getPhrase('chart_msg_must_be_event', 'Note selected must be an Event!'), true);

			}

			else showOutput(Language.getPhrase('chart_msg_select_single_event', 'You must select a single event to press this button.'), true);
...

		{
...

			{
...

				{
					...
					else showOutput(Language.getPhrase('chart_msg_no_event_deleted', 'No event is selected when you deleted it?? Weird.'), true);
		...
		tab_group.add(new FlxText(eventDropDown.x, eventDropDown.y - 15, 80, Language.getPhrase('chart_event_label', 'Event:')));

		tab_group.add(new FlxText(value1InputText.x, value1InputText.y - 15, 80, Language.getPhrase('chart_value1', 'Value 1:')));

		tab_group.add(new FlxText(value2InputText.x, value2InputText.y - 15, 80, Language.getPhrase('chart_value2', 'Value 2:')));
...

	{
		...
		tab_group.add(new FlxText(susLengthStepper.x, susLengthStepper.y - 15, 80, Language.getPhrase('chart_sustain_length', 'Sustain length:')));

		tab_group.add(new FlxText(strumTimeStepper.x, strumTimeStepper.y - 15, 100, Language.getPhrase('chart_note_hit_time', 'Note Hit time (ms):')));

		tab_group.add(new FlxText(noteTypeDropDown.x, noteTypeDropDown.y - 15, 80, Language.getPhrase('chart_note_type', 'Note Type:')));
...

	{
...

		{
...

			{
				...
				//showOutput(Language.getPhrase('chart_msg_unknown_section', 'ERROR: Unknown section??'), true);
...

			{
...

				{
					...
					showOutput(Language.getPhrase('chart_msg_nothing_to_copy', 'Nothing to copy!'), true);
		...
		mustHitCheckBox = new PsychUICheckBox(objX, objY, Language.getPhrase('chart_must_hit_sec', 'Must Hit Sec.'), 70, function()
		...
		gfSectionCheckBox = new PsychUICheckBox(objX + 100, objY, Language.getPhrase('chart_gf_section', 'GF Section'), 70, function()
		...
		altAnimSectionCheckBox = new PsychUICheckBox(objX + 200, objY, Language.getPhrase('chart_alt_anim', 'Alt Anim'), 70, function()
		...
		changeBpmCheckBox = new PsychUICheckBox(objX, objY, Language.getPhrase('chart_change_bpm', 'Change BPM'), 80, function()
		...
		var copyButton:PsychUIButton = new PsychUIButton(objX, objY, Language.getPhrase('chart_copy_section', 'Copy Section'), copyNotesOnSection.bind());

		var pasteButton:PsychUIButton = new PsychUIButton(objX + 100, objY, Language.getPhrase('chart_paste_section', 'Paste Section'), function()
		...
		affectNotes = new PsychUICheckBox(objX, objY, Language.getPhrase('chart_notes', 'Notes'), 60);

		affectNotes.checked = true;

		affectEvents = new PsychUICheckBox(objX + 100, objY, Language.getPhrase('chart_events', 'Events'), 60);



		objY += 32;

		var copyLastSecButton:PsychUIButton = new PsychUIButton(objX, objY, Language.getPhrase('chart_copy_last_section', 'Copy Last Section'), function()
		...
		var swapSectionButton:PsychUIButton = new PsychUIButton(objX, objY, Language.getPhrase('chart_swap_section', 'Swap Section'), function()
		...
		var duetSectionButton:PsychUIButton = new PsychUIButton(objX + 100, objY, Language.getPhrase('chart_duet_section', 'Duet Section'), function()
...

			{
...

				{
...

					{
						...
						showOutput(Language.getPhrase('chart_msg_cannot_dual_side', 'You cannot press this button with notes on more than one side.'));
		...
		var mirrorNotesButton:PsychUIButton = new PsychUIButton(objX + 200, objY, Language.getPhrase('chart_mirror_notes', 'Mirror Notes'), function()
		...
		tab_group.add(new FlxText(beatsPerSecStepper.x, beatsPerSecStepper.y - 15, 100, Language.getPhrase('chart_beats_per_section', 'Beats per Section:')));
...

	{
...

		{
...

			{
				...
				showOutput(Language.getPhrase('chart_msg_nothing_to_paste', 'Nothing to paste!'), true);
	...
	var tapTimestamps:Array<Float> = [];

	var tapButton:PsychUIButton;

	var analyzeButton:PsychUIButton;
...

	{
		...
		allowVocalsCheckBox = new PsychUICheckBox(objX, objY + 20, Language.getPhrase('chart_allow_vocals', 'Allow Vocals'), 80, function()
		...
		var reloadAudioButton:PsychUIButton = new PsychUIButton(objX + 120, objY, Language.getPhrase('chart_reload_audio', 'Reload Audio'), function() loadMusic(true), 80);



		#if mac

		var reloadJsonButton:PsychUIButton = new PsychUIButton(objX + 205, objY, Language.getPhrase('chart_reload_json', 'Reload JSON'), function()

		{
...

			{
				...
				showOutput(Language.getPhrase('chart_msg_not_psych_chart', 'Error: File loaded is not a Psych Engine/FNF 0.2.x.x chart.'), true);
...

			{
				...
				showOutput(Language.getPhrase('chart_msg_chart_opened', 'Opened chart \"{1}\" successfully!').replace('{1}', diff ? curdiff : cur));
		...
		// BPM Tap & Analyze buttons

		tapButton = new PsychUIButton(objX, objY + 25, Language.getPhrase('chart_tap', 'TAP'), function()

		{

			var now:Float = Sys.time() * 1000; // ms

			// Reset if last tap was more than 2 seconds ago

			if(tapTimestamps.length > 0 && (now - tapTimestamps[tapTimestamps.length - 1]) > 2000)

				tapTimestamps.resize(0);



			tapTimestamps.push(now);

			if(tapTimestamps.length > 8) // Keep last 8 taps max
...

			{
				...
				for(i in 1...tapTimestamps.length)

					totalDiff += tapTimestamps[i] - tapTimestamps[i - 1];

				var avgInterval:Float = totalDiff / (tapTimestamps.length - 1);

				var detectedBPM:Float = 60000 / avgInterval;



				// Clamp to reasonable range

				if(detectedBPM < 1) detectedBPM = 1;

				if(detectedBPM > 400) detectedBPM = 400;
				...
				// Halve/double to get into typical BPM range (60-300)

				while(detectedBPM > 300) detectedBPM = Math.round(detectedBPM / 2);

				while(detectedBPM < 60) detectedBPM = Math.round(detectedBPM * 2);
				...
				showOutput(Language.getPhrase('chart_msg_bpm_tap', 'BPM detected from TAP: {1} ({2} taps)').replace('{1}', Std.string(detectedBPM)).replace('{2}', Std.string(tapTimestamps.length)));
...

			{
				...
				showOutput(Language.getPhrase('chart_msg_tap_progress', 'TAP {1}/4...').replace('{1}', Std.string(tapTimestamps.length)));
		...
		analyzeButton = new PsychUIButton(objX + 70, objY + 25, Language.getPhrase('chart_analyse', 'ANALYSE'), function()
		...
		tab_group.add(new FlxText(songNameInputText.x, songNameInputText.y - 15, 80, Language.getPhrase('chart_song_name', 'Song Name:')));
		...
		tab_group.add(new FlxText(bpmStepper.x, bpmStepper.y - 15, 50, Language.getPhrase('chart_bpm', 'BPM:')));

		tab_group.add(new FlxText(scrollSpeedStepper.x, scrollSpeedStepper.y - 15, 80, Language.getPhrase('chart_scroll_speed', 'Scroll Speed:')));

		tab_group.add(new FlxText(audioOffsetStepper.x, audioOffsetStepper.y - 15, 100, Language.getPhrase('chart_audio_offset', 'Audio Offset (ms):')));
		...
		tab_group.add(tapButton);

		tab_group.add(analyzeButton);



		//dropdowns

		tab_group.add(new FlxText(stageDropDown.x, stageDropDown.y - 15, 80, Language.getPhrase('chart_stage', 'Stage:')));

		tab_group.add(new FlxText(playerDropDown.x, playerDropDown.y - 15, 80, Language.getPhrase('chart_player', 'Player:')));

		tab_group.add(new FlxText(opponentDropDown.x, opponentDropDown.y - 15, 80, Language.getPhrase('chart_opponent', 'Opponent:')));

		tab_group.add(new FlxText(girlfriendDropDown.x, girlfriendDropDown.y - 15, 80, Language.getPhrase('chart_girlfriend', 'Girlfriend:')));
...

	{
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, Language.getPhrase('chart_new', '  New'), function()
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, Language.getPhrase('chart_open_chart', '  Open Chart...'), function()

		{
...

			{
...

				{
					...
					showOutput(Language.getPhrase('chart_msg_error', 'Error: {1}').replace('{1}', e.message), true);
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, Language.getPhrase('chart_open_autosave', '  Open Autosave...'), function()

		{
...

			{
				...
				showOutput(Language.getPhrase('chart_msg_no_backups', 'The "backups" folder does not exist.'), true);
...

			{
				...
				showOutput(Language.getPhrase('chart_msg_no_autosaves', 'No autosave files found.'), true);
			...
			openSubState(new BasePrompt(420, hei, 'Choose an Autosave',
				...
				function(state:BasePrompt) {
					...
					var btn:PsychUIButton = new PsychUIButton(0, radioGrp.y + radioGrp.height + 20, Language.getPhrase('chart_load', 'Load'), function()
...

						{
...

							{
...

								{
									...
									showOutput(Language.getPhrase('chart_msg_not_autosave', 'Error: File loaded is not a valid Psych Engine autosave.'), true);
...

								{
									...
									showOutput(Language.getPhrase('chart_msg_autosave_opened', 'Opened autosave \"{1}\" successfully!').replace('{1}', autosaveName));
...

							{
								...
								showOutput(Language.getPhrase('chart_msg_autosave_error', 'Error on loading autosave: {1}').replace('{1}', e.message), true);

							}

						}

						else showOutput(Language.getPhrase('chart_msg_autosave_missing', 'Error! Autosave file selected could not be found, huh??'), true);
...

		{
			...
			var btn:PsychUIButton = new PsychUIButton(btnX, btnY, Language.getPhrase('chart_open_events', '  Open Events...'), function()
...

				{
...

					{
...

						{
							...
							showOutput(Language.getPhrase('chart_msg_not_events', 'Error: File loaded is not a Psych Engine chart/events file.'), true);
...

						{
							...
							showOutput(Language.getPhrase('chart_msg_events_empty', 'Events file loaded is empty.'), true);
						...
						openSubState(new BasePrompt('Events Found! Choose an action.',
...

							{
								...
								var btn:PsychUIButton = new PsychUIButton(0, btnY, Language.getPhrase('chart_replace_all', 'Replace All'), function()

								{
									...
									showOutput(Language.getPhrase('chart_msg_events_loaded', 'Events loaded successfully!'));
								...
								var btn:PsychUIButton = new PsychUIButton(0, btnY, Language.getPhrase('chart_add', 'Add'), function()

								{
									...
									showOutput(Language.getPhrase('chart_msg_events_added', 'Events added successfully!'));
								...
								var btn:PsychUIButton = new PsychUIButton(0, btnY, Language.getPhrase('chart_cancel', 'Cancel'), state.close);
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, Language.getPhrase('chart_save', '  Save'), function()
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, Language.getPhrase('chart_save_as', '  Save as...'), function()
...

		{
			...
			var btn:PsychUIButton = new PsychUIButton(btnX, btnY, Language.getPhrase('chart_save_events', '  Save Events...'), function()

			{
				...
				fileDialog.save('events.json', PsychJsonPrinter.print({events: PlayState.SONG.events, format: 'psych_v1'}, ['events']),
					...
					function() showOutput(Language.getPhrase('chart_msg_events_saved', 'Events saved successfully to: {1}').replace('{1}', fileDialog.path)), null,

					function() showOutput(Language.getPhrase('chart_msg_events_save_error', 'Error on saving events!'), true));
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, Language.getPhrase('chart_reload_chart', '  Reload Chart'), function()

		{
...

			{
...

				{
					...
					showOutput(Language.getPhrase('chart_msg_reload_first', 'You must save/load a Chart first to Reload it!'), true);
...

				{
...

					{
						...
						showOutput(Language.getPhrase('chart_msg_chart_reloaded', 'Chart reloaded successfully!'));
				...
				else showOutput(Language.getPhrase('chart_msg_reload_first', 'You must save/load a Chart first to Reload it!'), true);
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, Language.getPhrase('chart_save_vslice', '  Save (V-Slice)...'), function()
...

				{
					...
					openSubState(new BasePrompt('Metadata',
...

						{
...

								{
...

									{
										...
										if(overwriteSavedSomething)
											...
											showOutput(Language.getPhrase('chart_msg_files_saved', 'Files saved successfully to: {1}!').replace('{1}', path));
							...
							var btn:PsychUIButton = new PsychUIButton(btnX + 100, btnY, Language.getPhrase('chart_cancel', 'Cancel'), state.close);
							...
							var artistTxt:FlxText = new FlxText(artistInput.x, artistInput.y - 15, 100, Language.getPhrase('chart_artist', 'Artist/Composer:'));

							artistTxt.cameras = state.cameras;

							var charterTxt:FlxText = new FlxText(charterInput.x, charterInput.y - 15, 100, Language.getPhrase('chart_charter', 'Charter:'));
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, Language.getPhrase('chart_psych_to_vslice', '  Psych to V-Slice...'), function()

		{
...

			{
...

				{
					...
					showOutput(Language.getPhrase('chart_msg_chart_invalid', 'Error: Chart loaded is invalid.'), true);
...

					{
...

						{
...

							{
								...
								else showOutput(Language.getPhrase('chart_msg_need_difficulty', 'Error: You need at least one difficulty to export.'), true);
						...
						var difficultiesTxt:FlxText = new FlxText(difficultiesInput.x, difficultiesInput.y - 15, 100, Language.getPhrase('chart_difficulties', 'Difficulties:'));
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, Language.getPhrase('chart_vslice_to_psych', '  V-Slice to Psych...'), function()

		{
...

			{
...

				{
					...
					showOutput(Language.getPhrase('chart_msg_not_vslice_chart', 'Error: File loaded is not a valid FNF V-Slice chart.'), true);
...

				{
...

					{
						...
						showOutput(Language.getPhrase('chart_msg_not_vslice_meta', 'Error: File loaded is not a valid FNF V-Slice metadata.'), true);
...

					{
...

									{
...

										{
											...
											if(overwriteSavedSomething)
												...
												showOutput(Language.getPhrase('chart_msg_files_saved2', 'Files saved successfully to: {1}!').replace('{1}', fileDialog.path));
						...
						else showOutput(Language.getPhrase('chart_msg_no_difficulties', 'Error: No difficulties found.'));
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, Language.getPhrase('chart_update_legacy', '  Update (Legacy)...'), function()

		{
...

			{
...

				{
					...
					else showOutput(Language.getPhrase('chart_msg_already_uptodate', 'Chart is already up-to-date! Format: "{1}"').replace('{1}', fmt), true);
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, Language.getPhrase('chart_preview_f12', '  Preview (F12)'), openEditorPlayState, btnWid);
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, Language.getPhrase('chart_playtest_enter', '  Playtest (Enter)'), goToPlayState, btnWid);
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, Language.getPhrase('chart_exit', '  Exit'), function()
...

	{
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, Language.getPhrase('chart_undo', '  Undo'), undo, btnWid);
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, Language.getPhrase('chart_redo', '  Redo'), redo, btnWid);
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, Language.getPhrase('chart_select_all', '  Select All'), function()
...

		{
			...
			var btn:PsychUIButton = new PsychUIButton(btnX, btnY, Language.getPhrase('chart_lock_events_btn', '  Lock Events'), btnWid);
...

			{
				...
				if(lockedEvents) btn.text.text = Language.getPhrase('chart_unlock_events', '  Unlock Events');

				else btn.text.text = Language.getPhrase('chart_lock_events_btn', '  Lock Events');
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, Language.getPhrase('chart_autosave_settings', '  Autosave Settings...'), btnWid);
...

		{
			...
			openSubState(new BasePrompt(400, 160, 'Autosave Settings',
...

				{
					...
					checkbox = new PsychUICheckBox(timeStepper.x + 80, timeStepper.y, Language.getPhrase('chart_autosave_enabled', 'Enabled'), 60, function() {
					...
					var txt1:FlxText = new FlxText(timeStepper.x, timeStepper.y - 15, 100, Language.getPhrase('chart_time_minutes', 'Time (in minutes):'));

					txt1.cameras = state.cameras;

					var txt2:FlxText = new FlxText(maxFileStepper.x, maxFileStepper.y - 15, 100, Language.getPhrase('chart_file_limit', 'File Limit:'));
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, Language.getPhrase('chart_clear_all_notes', '  Clear All Notes'), function()
...

		{
			...
			var btn:PsychUIButton = new PsychUIButton(btnX, btnY, Language.getPhrase('chart_clear_all_events', '  Clear All Events'), function()
...

	{
		...
		vortexEditorButton = new PsychUIButton(btnX, btnY, vortexEnabled ? Language.getPhrase('chart_vortex_on', '  Vortex Editor ON') : Language.getPhrase('chart_vortex_off', '  Vortex Editor OFF'), function()

		{
			...
			vortexEditorButton.text.text = vortexEnabled ? Language.getPhrase('chart_vortex_on', '  Vortex Editor ON') : Language.getPhrase('chart_vortex_off', '  Vortex Editor OFF');
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, Language.getPhrase('chart_waveform', '  Waveform...'), function()

		{
			...
			openSubState(new BasePrompt(320, 200, 'Waveform Settings',
				...
				function(state:BasePrompt) {
					...
					var check:PsychUICheckBox = new PsychUICheckBox(state.bg.x + 40, state.bg.y + 80, Language.getPhrase('chart_waveform_enabled', 'Enabled'), 60);
					...
					var txt1:FlxText = new FlxText(input.x, input.y - 15, 80, Language.getPhrase('chart_color_hex', 'Color (Hex):'));
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, Language.getPhrase('chart_go_to', '  Go to...'), function()

		{
			...
			openSubState(new BasePrompt(420, 200, 'Go to Time/Section:',
...

				{
					...
					var txt1:FlxText = new FlxText(timeStepper.x, timeStepper.y - 15, 100, Language.getPhrase('chart_time_seconds', 'Time (in seconds):'));

					var txt2:FlxText = new FlxText(sectionStepper.x, sectionStepper.y - 15, 100, Language.getPhrase('chart_section_label', 'Section:'));
					...
					var btn:PsychUIButton = new PsychUIButton(0, timeTxt.y + 30, Language.getPhrase('chart_go_to_btn', 'Go To'), function()
					...
					var btn:PsychUIButton = new PsychUIButton(0, btn.y, Language.getPhrase('chart_cancel', 'Cancel'), state.close);
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, Language.getPhrase('chart_theme', '  Theme...'), function()

		{
			...
			openSubState(new BasePrompt(500, 260, 'Chart Editor Theme',
...

				{
					...
					var btn:PsychUIButton = new PsychUIButton(0, btnY, Language.getPhrase('chart_theme_light', 'Light'), changeTheme.bind(LIGHT));
					...
					var btn:PsychUIButton = new PsychUIButton(0, btnY, Language.getPhrase('chart_theme_dark', 'Dark'), changeTheme.bind(DARK));
					...
					var btn:PsychUIButton = new PsychUIButton(0, btnY, Language.getPhrase('chart_theme_default', 'Default'), changeTheme.bind(DEFAULT));
					...
					var btn:PsychUIButton = new PsychUIButton(0, btnY, Language.getPhrase('chart_theme_vslice', 'V-Slice'), changeTheme.bind(VSLICE));
					...
					var btn:PsychUIButton = new PsychUIButton(0, btnY, Language.getPhrase('chart_theme_custom', 'Custom'), changeTheme.bind(CUSTOM));
					...
					var txt:FlxText = new FlxText(input.x, input.y - 15, 120, Language.getPhrase('chart_bg_color', 'BG Color:'));
					...
					var txt:FlxText = new FlxText(input.x, input.y - 15, 120, Language.getPhrase('chart_grid_colors', 'Grid Colors:'));
					...
					var txt:FlxText = new FlxText(input.x, input.y - 15, 120, Language.getPhrase('chart_next_grid_colors', 'Next Grid Colors:'));
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, Language.getPhrase('chart_reset_ui', '  Reset UI Boxes'), function()
...

	{
...

		{
			...
			showOutput(Language.getPhrase('chart_msg_saved', 'Chart saved successfully to: {1}').replace('{1}', Song.chartPath));
...

		{
			...
			fileDialog.save(chartName, chartData,
...

				{
					...
					showOutput(Language.getPhrase('chart_msg_saved2', 'Chart saved successfully to: {1}').replace('{1}', newPath));



				}, null, function() showOutput(Language.getPhrase('chart_msg_save_error', 'Error on saving chart!'), true));
...

	{
...

		{
			...
			showOutput(Language.getPhrase('chart_msg_load_to_preview', 'Load a valid song to preview!'), true);
