...
class CreditsState extends MusicBeatState
{
	...
	override function create()
	{
		...
		var defaultList:Array<Array<String>> = [ //Name - Icon name - Description - Link - BG Color
			["Unknown Engine Team"],
			["Pandaman",      		"pdm",	        	"Programmer and Head of Unknown Engine",					"null",	"444444"],
			[""],
		...
		changeSelection(0, false); // skip sound on initial load
		super.create();
	}
	...
	override function update(elapsed:Float)
	{
		...
		{
			if(item.bold || Math.abs(item.targetY) > 15) continue;
	...
	function changeSelection(change:Int = 0, ?skipSound:Bool = false)
	{
		if (!skipSound)
