...
class VisualsSettingsSubState extends BaseOptionsMenu
{
	...
	public function new()
	{
		...
		var option:Option = new Option('Rainbow FPS',
			'If checked, FPS Counter will display in cycling rainbow colors.',
			'colorfulFPS',
			BOOL);
		addOption(option);
		option.onChange = onChangeColorfulFPS;

		var option:Option = new Option('Rainbow FPS Speed',
			'How fast the rainbow colors cycle.',
			'colorfulFPSSpeed',
			PERCENT);
		...
		option.decimals = 0;
		addOption(option);
		option.onChange = onChangeColorfulFPSSpeed;
		#end
	...
	function onChangeColorfulFPS()
	{
		if(Main.fpsVar != null)
			Main.fpsVar.setColorfulFPS(ClientPrefs.data.colorfulFPS);
	}

	function onChangeColorfulFPSSpeed()
	{
		if(Main.fpsVar != null)
			Main.fpsVar.setColorfulFPSSpeed(ClientPrefs.data.colorfulFPSSpeed);
	}
