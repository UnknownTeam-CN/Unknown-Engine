...
// import animateatlas.AtlasFrameMaker; // unavailable in UE 1.0
import flixel.FlxG;
...
class LegacyFunkinLua {
	...
	#if hscript
	public static var hscript:LegacyHScript = null;
	#end
	...
	public function new(script:String) {
		...
		var difficultyName:String = Difficulty.getString(false);
		...
		Lua_helper.add_callback(lua, "openCustomSubstate", function(name:String, pauseGame:Bool = false) {
			...
			PlayState.instance.openSubState(new LegacyCustomSubstate(name));
		});

		Lua_helper.add_callback(lua, "closeCustomSubstate", function() {
			if(LegacyCustomSubstate.instance != null)
			{
				PlayState.instance.closeSubState();
				LegacyCustomSubstate.instance = null;
				return true;
			}
			return false;
		});
		...
		Lua_helper.add_callback(lua, "keyJustPressed", function(name:String) {
			...
			switch(name) {
				...
				case 'accept': key = PlayState.instance.controls.ACCEPT;
				case 'back': key = PlayState.instance.controls.BACK;
				case 'pause': key = PlayState.instance.controls.PAUSE;
				case 'reset': key = PlayState.instance.controls.RESET;
		...
		Lua_helper.add_callback(lua, "exitSong", function(?skipTransition:Bool = false) {
			...
			// WeekData.loadTheFirstEnabledMod(); // unavailable in 1.0.4
			return true;
		});
		...
		Lua_helper.add_callback(lua, "makeLuaSprite", function(tag:String, image:String, x:Float, y:Float) {
			tag = tag.replace('.', '');
			resetSpriteTag(tag);
			var leSprite:FlxSprite = new LegacyModchartSprite(x, y);
		...
		Lua_helper.add_callback(lua, "playAnim", function(obj:String, name:String, forced:Bool = false, ?reverse:Bool = false, ?startFrame:Int = 0)
		{
			if(PlayState.instance.getLuaObject(obj, false) != null) {
				...
				if(luaObj.animation.getByName(name) != null)
				{
					...
					if(Std.isOfType(luaObj, LegacyModchartSprite))
					{
		...
		Lua_helper.add_callback(lua, "setTimeBarColors", function(leftHex:String, rightHex:String) {
			...
			PlayState.instance.timeBar.setColors(right, left);
		...
		Lua_helper.add_callback(lua, "makeLuaText", function(tag:String, text:String, width:Int, x:Float, y:Float) {
			tag = tag.replace('.', '');
			resetTextTag(tag);
			var leText:LegacyModchartText = new LegacyModchartText(x, y, text, width);
		...
		Lua_helper.add_callback(lua, "addLuaText", function(tag:String) {
			if(PlayState.instance.modchartTexts.exists(tag)) {
				var shit:LegacyModchartText = PlayState.instance.modchartTexts.get(tag);
				if(!shit.wasAdded) {
					getInstance().add(shit);
					shit.wasAdded = true;
					//trace('added a thing: ' + tag);
		...
		Lua_helper.add_callback(lua, "removeLuaText", function(tag:String, destroy:Bool = true) {
			...
			var pee:LegacyModchartText = PlayState.instance.modchartTexts.get(tag);
			if(destroy) {
				pee.kill();
			}
	...
	public function initHaxeModule()
	{
		if(hscript == null)
		{
			...
			hscript = new LegacyHScript(); //TO DO: Fix issue with 2 scripts not being able to use the same variable names
		}
	}
	#end
...
class LegacyModchartSprite extends FlxSprite
{
...
class LegacyModchartText extends FlxText
{
	public var wasAdded:Bool = false;
...
class LegacyDebugLuaText extends FlxText
{
	private var disableTime:Float = 6;
	public var parentGroup:FlxTypedGroup<LegacyDebugLuaText>;
	public function new(text:String, parentGroup:FlxTypedGroup<LegacyDebugLuaText>, color:FlxColor) {
...
class LegacyCustomSubstate extends MusicBeatSubstate
{
	public static var name:String = 'unnamed';
	public static var instance:LegacyCustomSubstate;
	...
	{
		LegacyCustomSubstate.name = name;
		super();
